...
class DynamicColorPlaybackTest : WearScreenshotTest() {
  ...
  private fun assertCenterPixel(expected: Color) {
    ...
    val screenshot = File.createTempFile("dynamic-color-", ".png")
    try {
      composeRule
        .onNodeWithTag("animation")
        .captureRoboImage(
      ...
      try {
        assertThat(bitmap.getPixel(bitmap.width / 2, bitmap.height / 2))
          .isEqualTo(expected.toArgb())
      } finally {
        bitmap.recycle()
      }
    } finally {
