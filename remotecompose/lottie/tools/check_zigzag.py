#!/usr/bin/env python3
"""Audit the offline lottie-web zigzag playback captures and make review sheets.

Run ZigZagRegressionTest first. Reference coordinates come from the pinned web
...
def main():
    ...
    fixtures = json.loads((module / "src/test/resources/zigzag-reference.json").read_text())
    errors, tiles = [], []
    count = 0
    for case in fixtures["motion"]:
        folder = module / "build/outputs/lottie-motion" / ("zigzag-" + case["name"])
        ...
        for frame in case["frames"]:
            ...
            with Image.open(paths[0]) as ref, Image.open(paths[1]) as rc:
                ...
                if mean > 0.00005 or peak > 0.25 or sum(p[0] > 127 for p in a) <= 20:
        ...
        if args.contact_sheet_prefix:
            ...
            for column, frame in enumerate((0, 5, 10)):
    ...
    if errors:
        raise SystemExit("\n".join(errors))
    print(f"PASS: {len(fixtures['motion'])} motion cases, {count} frame pairs")
