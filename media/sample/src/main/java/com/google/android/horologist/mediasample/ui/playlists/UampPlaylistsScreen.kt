...
fun UampPlaylistsScreen(
    ...
    val modifiedState = when (uiState) {
        is PlaylistsScreenState.Loaded -> {
            val modifiedPlaylistList =
                (uiState as PlaylistsScreenState.Loaded<PlaylistUiModel>).playlistList.map {
    ...
    // b/242302037 — stop listening to uiState emissions while dialog is presented
    AlertDialog(
        visible = modifiedState == PlaylistsScreenState.Failed,
